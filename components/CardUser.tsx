...
import { capitalizeFirstLetters } from '@/lib/actions';
...
const CardUser = ({ user}: Props) => {
  ...
  return (
    <div 
      onClick={handleClick}
      className={`flex w-full ${user.pendingRequest ? 'animate-pulse' : ''}`}
      >
      <div 
        className='flex w-full max-h-fit gap-2 m-1 rounded overflow-hidden cursor-pointer shadow hover:border-slate-300/50' 
        >
        <Image
